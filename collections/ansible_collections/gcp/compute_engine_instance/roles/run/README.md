Gcp.Compute_engine_instance Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: gcp.compute_engine_instance.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from gcp.compute_engine_instance
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: gcp.compute_engine_instance.run
      vars:
        run_x: 42
```
